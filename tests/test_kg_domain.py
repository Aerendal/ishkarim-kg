"""
test_kg_domain.py — testy domenowe dla ishkarim_kg.
...
import ishkarim_kg
from ishkarim_kg import load_knowledge_index


@pytest.fixture
def repo_root():
    """Zwraca ścieżkę do katalogu głównego repozytorium Ishkarim."""
    root = Path(__file__).resolve().parents[3]  # projects/ishkarim-kg/tests/ → repo root
...
class TestModuleIntegrity:
    ...
    def test_version_format(self):
        assert re.match(r"\d+\.\d+\.\d+", ishkarim_kg.__version__)

    def test_modules_all_strings(self):
        for name in ishkarim_kg.MODULES:
